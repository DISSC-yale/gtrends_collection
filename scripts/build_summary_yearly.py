...
if __name__ == "__main__":
    ...
    data = dataset("data_yearly").to_table().to_pandas()
    ...
    observations.to_csv("summaries/observations_yearly.csv")
    ...
    means.to_csv("summaries/means_yearly.csv")
    ...
    with open("docs_source/Data_Yearly.md", "w", encoding="utf-8") as out:
        out.writelines(
            [
                "Summaries of the data collected as of "
                + datetime.datetime.now(datetime.timezone.utc).strftime("%I:%M:%S %p UTC on %Y-%m-%d")
                + f"\n\n## Locations\n\n* [Observations]({summaries_url}observations_yearly.csv)"
                + f"\n* [Means]({summaries_url}means_yearly.csv)"
                + "\n\n## Dates\n\n",
                dates.to_markdown(),
                "\n\n## Values\n\n",
